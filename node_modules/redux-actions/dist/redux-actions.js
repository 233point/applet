...
var camelCase = (function (type) {
  return type.indexOf(namespacer) === -1 ? toCamelCase_1(type) : type.split(namespacer).map(function (part) {
    return toCamelCase_1(part);
  }).join(namespacer);
});
...
function handleActions(handlers, defaultState, options) {
  ...
  var reducers = ownKeys(flattenedReducerMap).map(function (type) {
    return handleAction(type, get$1(type, flattenedReducerMap), defaultState);
  });
